...
from .models import Superadmin,Account,Notification
from .forms import AccountForm,AdminForm,NotificationForm
...
class NotificationAPI(UnifispotAPI):

    ''' API class to deal with Notifications
    ...
    def __init__(self):
        super(self.__class__, self).__init__()
        self.columns = []
        self.entity_name = 'Notifications'
    ...
    def get(self,id):   
        current_app.logger.debug("User with ID:%s is trying to access Notifications API with GET :%s "%(current_user.id,request.url))  
        return jsonify({'status': 0,'msg':'Not Authorized'}) 

  
    def post(self,id):  
        if not id:
            return jsonify({'status': 0,'msg':'POST without any notification ID specified'}) 

        mark = Notification.mark_as_read(id,current_user.account.id)
        if mark:
    ...
    def delete(self,id):   
        current_app.logger.debug("User with ID:%s is trying to access Notifications API with DELTE :%s "%(current_user.id,request.url))  
        return jsonify({'status': 0,'msg':'Not Authorized'}) 
